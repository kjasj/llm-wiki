...
Agent 层：Runtime / Loop / State / Memory / Skill / Evaluator
...
| 概念 | 一句话解释 | 继续读 |
| --- | --- | --- |
| Prefill | 处理输入 prompt，并为后续生成建立 KV Cache | [Chat API 执行链路](openai-chat-api-execution-pipeline.md) |
| Decode | 在单序列内逐 token 生成，并可跨序列组成 batch | [Chat API 执行链路](openai-chat-api-execution-pipeline.md) |
...
| Operator Fusion | 把多个小算子合并，减少 kernel launch 和 HBM 往返 | [Hy3 Preview 推理优化案例](hy3-preview-inference-optimization-case.md) |
| Sparse Attention | 只计算部分关键 attention，降低长上下文 prefill 成本 | [Hy3 Preview 推理优化案例](hy3-preview-inference-optimization-case.md) |
| Multi-level KV Cache | 把 KV / Prefix Cache 分层放在 GPU、CPU 和外部存储 | [Hy3 Preview 推理优化案例](hy3-preview-inference-optimization-case.md) |
...
| Sequence Parallel | 沿序列维度切分计算，减少长 prefill 冗余 | [Hy3 Preview 推理优化案例](hy3-preview-inference-optimization-case.md) |
...
| Chat Completions | 经典 messages 风格聊天接口 | [Chat API 执行链路](openai-chat-api-execution-pipeline.md) |
| OpenAI-compatible | 接口形状尽量兼容 OpenAI，但底层实现可以不同 | [Chat API 执行链路](openai-chat-api-execution-pipeline.md) |
| Chat Template | 把 messages、tools 和角色标记渲染成模型训练格式 | [Chat API 执行链路](openai-chat-api-execution-pipeline.md) |
| Streaming / SSE | 模型生成时由返回层持续发送增量 | [Chat API 执行链路](openai-chat-api-execution-pipeline.md) |
...
| RAG | 检索资料后再生成回答 | [RAG 工程实践](rag-engineering-practice.md) |
| Embedding | 用向量表示文本语义 | [RAG 工程实践](rag-engineering-practice.md) |
| Reranker | 对检索结果重排 | [RAG 工程实践](rag-engineering-practice.md) |
| Model Router | 按任务、成本、延迟和风险选择模型 | [模型选择与路由](model-selection-routing.md) |
| Model Gateway | 统一模型供应商、模型别名、重试、fallback 和成本统计 | [LLM 应用生产化](llmops-production.md) |
| LLMOps | LLM 应用的评测、发布、监控、成本和回滚体系 | [LLM 应用生产化](llmops-production.md) |
...
| Agent Runtime | 管理上下文、工具、权限、沙箱、Skills 和 trace 的运行时 | [Agent 项目开发实战](agent-runtime-project-development.md) |
| Agent Loop | 思考、行动、观察、更新、继续或停止 | [Loop Engineering](loop-engineering.md) |
| Context Trimming | 在预算不足时裁剪低价值上下文 | [Agent 项目开发实战](agent-runtime-project-development.md) |
| Context Compression | 把长历史压缩成可恢复摘要 | [Agent 项目开发实战](agent-runtime-project-development.md) |
...
| readFile / writeFile / editFile | Agent 读写和局部编辑文件的默认工具 | [Agent 项目开发实战](agent-runtime-project-development.md) |
| Bash / glob / grep | Agent 执行命令、找文件和搜索文本的默认工具 | [Agent 项目开发实战](agent-runtime-project-development.md) |
| MCP | Agent 连接外部工具、数据和资源的协议 | [MCP 工具协议](mcp-tool-protocol.md) |
...
| Sandbox | 在执行层限制文件、网络、命令和资源 | [Agent 项目开发实战](agent-runtime-project-development.md) |
...
| “一次 Chat API 请求在服务端怎么执行？” | [OpenAI-compatible Chat API 执行链路](openai-chat-api-execution-pipeline.md) |
...
| “RAG 为什么答不准？” | [RAG 工程实践](rag-engineering-practice.md) |
| “不同模型应该怎么选？” | [模型选择与路由](model-selection-routing.md) |
...
| “LLM 应用上线要补什么？” | [LLM 应用生产化](llmops-production.md) |
| “Agent 为什么会停不下来？” | [Loop Engineering](loop-engineering.md) |
| “Agent runtime 怎么实现？” | [Agent 项目开发实战](agent-runtime-project-development.md) |
| “上下文裁剪和压缩怎么做？” | [Agent 项目开发实战](agent-runtime-project-development.md) |
| “Agent 默认工具怎么设计？” | [Agent 项目开发实战](agent-runtime-project-development.md) |
| “权限和沙箱怎么落地？” | [Agent 项目开发实战](agent-runtime-project-development.md) |
| “工具调用和 Agent 有什么区别？” | [LLM 应用架构](llm-application-architecture.md) |
| “MCP 和工具调用有什么区别？” | [MCP 工具协议](mcp-tool-protocol.md) |
